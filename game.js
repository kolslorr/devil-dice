...
var puzzleMovesRemaining = 0, puzzleCleared = false;
var aiTickInterval = null, aiLockedBlockQueue = [];
...
var AudioEngine = {
    ...
    playRoll: function() { this.playTone(180, 'sawtooth', 0.15, 0.15, 80); },
...
function checkAllMatches() {
    ...
    for (var x = 0; x < GRID_COLS; x++) for (var y = 0; y < GRID_ROWS; y++) {
        ...
        if (component.length >= targetVal) { matchedAny = true; var groupId = Date.now() + Math.random().toString(36).substr(2, 5);
            ...
            var points = targetVal * component.length * 100; score += points; updateScoreDisplay(); AudioEngine.playMatch();
            createFloatingScore(component[0].gridX, component[0].gridY, '+' + points); triggerChainEvaluations(groupId, targetVal);
            if (gameMode === 'battle') battleSendLockedBlocks(1); } }
    checkOnesChaining();
}
...
function triggerRoll(die, dir) { animationLock = true; die.roll(dir, function() { evaluateRollChain(die); checkAllMatches(); animationLock = false; if (gameMode === 'puzzle') decrementPuzzleMove(); }); }
function triggerSlide(die, dir) { animationLock = true; die.slide(dir, function() { evaluateRollChain(die); checkAllMatches(); animationLock = false; if (gameMode === 'puzzle') decrementPuzzleMove(); }); }
function handleKeyboard(e) { if (gameState !== 'playing') return; var k = e.key.toLowerCase(); if (k === 'p' || k === 'escape') { pauseGame(); return; } var dir = null; if (k === 'arrowup' || k === 'w') dir = 'north'; else if (k === 'arrowdown' || k === 's') dir = 'south'; else if (k === 'arrowleft' || k === 'a') dir = 'west'; else if (k === 'arrowright' || k === 'd') dir = 'east'; else return; var die = findRollableDie(); if (die) triggerRoll(die, dir); }
...
function quitToMenu() { gameState = 'menu'; stopSpawning(); AudioEngine.stopBGM(); if (gameMode === 'battle') stopAITicks(); document.getElementById('pause-screen').classList.remove('active'); document.getElementById('gameover-screen').classList.remove('active'); document.getElementById('hud-screen').classList.remove('active'); document.getElementById('ai-canvas-container').style.display = 'none'; document.getElementById('menu-screen').classList.add('active'); document.getElementById('menu-highscore').innerText = Number(highScore).toLocaleString(); clearAllDice(); }
function clearAllDice() { for (var x = 0; x < GRID_COLS; x++) for (var y = 0; y < GRID_ROWS; y++) if (grid[x] && grid[x][y]) { diceGroup.remove(grid[x][y].pivotGroup); grid[x][y].mesh.geometry.dispose(); } grid = Array(GRID_COLS).fill(null).map(function() { return Array(GRID_ROWS).fill(null); }); activeSinkingGroups = []; }
function triggerGameOver() { gameState = 'gameover'; stopSpawning(); AudioEngine.stopBGM(); AudioEngine.playGameOver(); if (gameMode === 'battle') stopAITicks(); var isNewHigh = false; if (score > highScore) { highScore = score; localStorage.setItem('devildice_zen_hs', highScore); isNewHigh = true; } document.getElementById('go-score').innerText = score.toLocaleString(); document.getElementById('go-combo').innerText = comboCount.toString(); document.getElementById('new-high-indicator').style.display = isNewHigh ? 'block' : 'none'; var titleEl = document.getElementById('gameover-title-text'); if (gameMode === 'puzzle') titleEl.innerText = puzzleCleared ? 'BOARD CLEARED!' : 'OUT OF MOVES!'; else if (gameMode === 'battle') titleEl.innerText = 'DEFEATED!'; else titleEl.innerText = 'BOARD FILLED!'; document.getElementById('hud-screen').classList.remove('active'); document.getElementById('gameover-screen').classList.add('active'); document.getElementById('ai-canvas-container').style.display = 'none'; }

function setupPuzzleMode() { clearAllDice(); puzzleCleared = false; puzzleMovesRemaining = 15 + Math.floor(Math.random() * 10); document.getElementById('hud-moves').innerText = String(puzzleMovesRemaining).padStart(2, '0'); var puzzleLayout = [{ x: 1, y: 2, v: 3 }, { x: 2, y: 2, v: 3 }, { x: 3, y: 2, v: 3 }, { x: 4, y: 4, v: 2 }, { x: 5, y: 4, v: 2 }, { x: 1, y: 5, v: 4 }, { x: 2, y: 5, v: 4 }, { x: 3, y: 5, v: 4 }, { x: 4, y: 5, v: 4 }, { x: 2, y: 3, v: 1 }, { x: 4, y: 3, v: 1 }, { x: 3, y: 4, v: 5 }, { x: 5, y: 2, v: 2 }, { x: 1, y: 1, v: 6 }, { x: 2, y: 1, v: 6 }, { x: 3, y: 1, v: 6 }]; var placed = new Set(); puzzleLayout.forEach(function(item) { var key = item.x + ',' + item.y; if (!placed.has(key) && item.x < GRID_COLS && item.y < GRID_ROWS) { placed.add(key); var d = new Die(item.x, item.y, item.v); d.state = 'normal'; d.height = 0; d.mesh.material = getDiceMaterials(d.faces, 'normal'); d.updateMeshPosition(); grid[item.x][item.y] = d; } }); checkAllMatches(); updateFullnessBar(countActiveDice() / totalCells); }
function decrementPuzzleMove() { puzzleMovesRemaining--; document.getElementById('hud-moves').innerText = String(Math.max(0, puzzleMovesRemaining)).padStart(2, '0'); if (puzzleMovesRemaining <= 0) { if (countActiveDice() === 0) { puzzleCleared = true; score += 5000; } triggerGameOver(); } else if (countActiveDice() === 0) { puzzleCleared = true; score += puzzleMovesRemaining * 500; triggerGameOver(); } }

function setupBattleMode() { clearAllDice(); aiLockedBlockQueue = []; document.getElementById('battle-ai-status').innerText = 'ACTIVE'; populateInitialBoard(); startSpawning(); startAITicks(); }
function stopAITicks() { if (aiTickInterval) { clearInterval(aiTickInterval); aiTickInterval = null; } }
function startAITicks() { stopAITicks(); var rates = { easy: 1200, medium: 800, hard: 500 }, interval = rates[aiDifficulty] || 800; aiTickInterval = setInterval(function() { if (gameState !== 'playing' || gameMode !== 'battle') return; while (aiLockedBlockQueue.length > 0) { aiLockedBlockQueue.shift(); var emptySlots = []; for (var x = 0; x < GRID_COLS; x++) for (var y = 0; y < GRID_ROWS; y++) if (grid[x][y] === null) emptySlots.push({ x: x, y: y }); if (emptySlots.length > 0) { var slot = emptySlots[Math.floor(Math.random() * emptySlots.length)], lb = new Die(slot.x, slot.y, null, CELL_TYPE.LOCKED); grid[slot.x][slot.y] = lb; AudioEngine.playLockBlock(); } } var bestMove = aiFindBestMove(); if (bestMove) { if (bestMove.isRoll) bestMove.die.roll(bestMove.direction, function() {}); else bestMove.die.slide(bestMove.direction, function() {}); setTimeout(function() { if (gameState === 'playing' && gameMode === 'battle') checkAllMatches(); }, ROLL_DURATION + 50); } }, interval); }
...
function battleSendLockedBlocks(n) { n = n || 1; if (gameMode !== 'battle') return; for (var i = 0; i < n; i++) aiLockedBlockQueue.push({}); }
